...
char validity(char tic[9])
{
    ...
    if(ifvalid(tic,'O')==true)//If Player 2 Wins and other valid condition
    {
        if(Xc != Oc)
        {
            return 'I';
    ...
    if(ifvalid(tic,'X')==true)//Player 1 wins and the other valid condition
    {
        
        if(Xc != Oc+1)//Validity if 'X' wins as 'X' starts the game
        {
            return 'I';
        }
...
{
    int t,i;
    cout<<"Enter No of Times You want to play"<<endl;
    char k;
    cin>>t;
    while(t--)
    {
